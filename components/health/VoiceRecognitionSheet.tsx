...
/**
 * VoiceRecognitionSheet — uses the Web Speech API embedded in a WebView.
 * Works on iOS, Android (Chrome engine), and web.
 * When a result comes in, it's posted back to React Native via postMessage
 * and parsed by the caller (health.tsx) using the existing voice text parser.
 */
...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; font-family: -apple-system, system-ui, sans-serif; }
    body { background: transparent; display: flex; flex-direction: column; align-items: center; justify-content: center; min-height: 100vh; padding: 24px; }
...
<body>
  ...
  <script>
    ...
    if (!SpeechRecognition) {
      document.getElementById('error').textContent = '⚠️ Voice recognition not supported in your browser. Please type instead.';
      ...
      recognition.onend = () => {
        ...
        if (finalTranscript) {
          document.getElementById('status').textContent = '✅ Got it! Processing...';
          if (window.ReactNativeWebView) {
            window.ReactNativeWebView.postMessage(JSON.stringify({ type: 'result', transcript: finalTranscript }));
          } else {
            // Web mode
      ...
      recognition.onerror = (event) => {
        ...
        if (event.error === 'not-allowed') {
          document.getElementById('error').textContent = '🚫 Microphone permission denied. Please allow access in your browser settings.';
        }
      };
    }
...
`;
  ...
  const handleWebViewMessage = (event: any) => {
    try {
      ...
      if (data.type === 'result' && data.transcript) {
        onResult(data.transcript);
      }
    } catch {}
  };

  // Web platform: use the same Web Speech API directly via a hidden iframe
  if (Platform.OS === 'web') {
    return (
  ...
  return (
    <Modal visible={visible} transparent animationType="slide">
      <View style={styles.overlay}>
        <LinearGradient colors={['#1e293b', '#0f172a']} style={styles.sheet}>
          <View style={styles.sheetHeader}>
...
const styles = StyleSheet.create({
  ...
  webview: { width: '100%', height: 360, borderRadius: 16, overflow: 'hidden', backgroundColor: 'transparent' },
